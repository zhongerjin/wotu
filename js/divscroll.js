						...
						C.ontouchstart = function(e) {
							...
							if (!C.locked) {
								...
								if (!D.cantouch && !this.istouchcapable && !D.hasmstouch) {
									var g = (a) ? /INPUT|BUTTON|SELECT|TEXTAREA|A/i.test(a.nodeName) : false;
									var _class = (a) ? /radio_diy|modal/i.test(a.className) : false;
									if (!g && !_class) {
										if (!C.ispage && D.hasmousecapture) a.setCapture();
										return C.cancelEvent(e)
									}
									if (/SUBMIT|CANCEL/i.test($(a).attr('type'))) {
										pc = {
											"tg": a,
											"click": false
										};
